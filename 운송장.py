...
import time
import pandas as pd
import cx_Oracle  # Oracle DB와의 연결을 위한 라이브러리 추가
...
# Oracle 데이터베이스에 연결
dsn = cx_Oracle.makedsn('localhost', 1521, service_name='xe')  # DSN 생성
connection = cx_Oracle.connect(user='system', password='pass', dsn=dsn)  # Oracle DB 연결
...
def save_to_db(df):
    ...
    for index, row in df.iterrows():
        cursor.execute('''
            INSERT INTO 배송정보 
            (운송번호, 받는분이름, 받는분휴대폰번호, 받는분주소, 보내는분이름, 보내는분휴대폰번호, 보내는분주소, 메모)
